...
class UndergroundSystem {
    private HashMap<Integer, StartStamp> recordsInProgress;
    private HashMap<String, List<Integer>> finishedTrips;
    class StartStamp {
        String startStation;
        int time;
    }
    ...
    public UndergroundSystem() {
        recordsInProgress = new HashMap<>();
        finishedTrips = new HashMap<>();
    }

    public void checkIn(int id, String stationName, int t) {
        StartStamp stamp = new StartStamp();
        stamp.startStation = stationName;
        stamp.time = t;
        recordsInProgress.put(id, stamp);
    }

    public void checkOut(int id, String stationName, int t) {
        StartStamp stamp = recordsInProgress.get(id);
        String key = stamp.startStation + "-" + stationName;
        List<Integer> existingTripTimes = finishedTrips.getOrDefault(key, new ArrayList<>());
        existingTripTimes.add(t - stamp.time);
        finishedTrips.put(key, existingTripTimes);
    }

    public double getAverageTime(String startStation, String endStation) {
        String key = startStation + "-" + endStation;
        List<Integer> existingTrips = finishedTrips.getOrDefault(key, new ArrayList<>());
        double averageTime = existingTrips.stream()
                .mapToDouble(d->d)
                .average().getAsDouble();
        return averageTime;
    }
