...
public class CandyCrush {
    public int[][] candyCrush(int[][] board) {
        boolean crushable = true;
        while (crushable) {
            crushable = crushCrushableCandies(board);
            gravityPull(board);
        }
        return board;
    }

    private boolean crushCrushableCandies(int[][] board) {
        boolean crushed = false;
        int height = board.length;
        int width = board[0].length;
//        boolean [][] tempMarks = new boolean[height][width];
        for(int i = 0; i < height; i++) {
            for(int j = 0; j < width - 2; j++) {
                int currentVal = Math.abs(board[i][j]);
                if(currentVal != 0 && currentVal == Math.abs(board[i][j+1]) && currentVal == Math.abs(board[i][j+2])) {
//                    tempMarks[i][j] = tempMarks[i][j+1] = tempMarks[i][j+1] = true;
                    board[i][j] = board[i][j+1] = board[i][j+2] = -currentVal;
                    crushed = true;
                }
            }
        }
        for(int j = 0; j < width; j++) {
            for(int i = 0; i < height - 2; i++) {
                int currentVal = Math.abs(board[i][j]);
                if(currentVal != 0 && currentVal == Math.abs(board[i+1][j]) && currentVal == Math.abs(board[i+2][j])) {
//                    tempMarks[i][j] = tempMarks[i+1][j] = tempMarks[i+2][j] = true;
                    board[i][j] = board[i+1][j] = board[i+2][j] = -currentVal;
                    crushed = true;
                }
            }
        }
    ...
    private void gravityPull(int[][] board) {
        int numOfRows = board.length;
        int numOfCols = board[0].length;
        List<Queue<Integer>> listOfQueues = new ArrayList<>();
        ...
        //Dump the elements from list of stacks back to original array
        for(int c = 0; c < numOfCols; c++) {
            Queue queue = listOfQueues.get(c);
            for(int r = numOfRows - 1; r >= 0; r--) {
                if(!queue.isEmpty()) {
                    board[r][c] = (Integer) queue.poll();
                } else {
                    board[r][c] = 0;
                }
            }
    ...
    public static void main(String[] args) {
        ...
        int[][] result = solution.candyCrush(board_1);
        for(int i = 0; i < result.length; i++) {
            for(int j = 0; j < result[0].length; j++) {
                if(result[i][j] <= 9) {
                    System.out.print("  "+ result[i][j] + ",");
                } else if(result[i][j] < 100 && result[i][j] > 9) {
                    System.out.print(" "+ result[i][j] + ",");
                } else {
                    System.out.print(result[i][j] + ",");
                }
