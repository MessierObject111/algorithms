...
public class OpenTheLock {
    public int openLock(String[] deadends, String target) {
        // Grammar 1:
//        boolean contains = Arrays.stream(deadends).anyMatch("0000"::equals);
        // Grammar 2:

        boolean contains = Arrays.stream(deadends).anyMatch(ele -> ele.equals("0000"));
        // If there is a dead end at the start point, return 0.
        if (contains) {
            return 0;
        }
        // Check each wheel to see if there are blockers from both ends. For example, if start is "0", target is "7",
        // and there are dead ends exists in both the 1 -> 6 part and 8 -> 9 part, it will be impossible for us to reach
        // the target.
        for(int i = 0; i < 4; i++) {
            int start = 0;
            int end = (int) target.charAt(i);
    ...
    public static void main(String[] args) {
        String[] deadends = {"0201","0101","0102","1212","2002", "0000"};
        String target = "0202";
